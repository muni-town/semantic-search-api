...
impl Engine {
    ...
    pub async fn index<I: IntoIterator<Item = Item>>(&mut self, items: I) -> anyhow::Result<()> {
        ...
        let embeddings = self.model.embed(texts.clone(), Some(BATCH_SIZE))?;
        ...
        let points = id_payloads
            ...
            .collect::<Vec<_>>();
        self.qdrant
            .upsert_points(UpsertPointsBuilder::new(&self.qdrant_collection, points))
            .await?;
